  ...
  it('should fail the lazily query an unknown SPV', async () => {
    ...
    try {
      ...
      expect(error.message).to.match(
        /The subscriber package version 04txxxxxxxxxxxxxxy is either invalid or not yet available on your instance of salesforce.com. {2}First double-check the ID to ensure it's correct. {2}If it is, check back after a while and retry the package install./,
        error.message
      );
      expect(queryStub.called).to.be.true;
    }
