...
describe('Package Version Create', () => {
  ...
  it('should replace "New - Not Contacted" with "Open - Not Contacted" in Lead.object when edition is Developer', async () => {
    const minimalPackageXml =
      '<?xml version="1.0"?><Package xmlns="http://soap.sforce.com/2006/04/metadata"><types><members>MyApexClass</members><name>ApexClass</name></types><version>58.0</version></Package>';
    const scratchOrgDefFileContent = JSON.stringify({
      Edition: 'Developer',
      objectSettings: { lead: { defaultRecordType: 'default' } },
    });
    const scratchOrgDefFileName = 'project-scratch-def.json';
    const originalReadFile = fs.promises.readFile.bind(fs.promises);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument
    $$.SANDBOX.stub(fs.promises, 'readFile').callsFake((filePath: any, ...args: any[]) => {
      const pathStr = String(filePath);
      if (pathStr === scratchOrgDefFileName) return Promise.resolve(scratchOrgDefFileContent);
      if (pathStr.endsWith('package.xml')) return Promise.resolve(minimalPackageXml);
    ...
    const leadWriteCall = writeFileSpy.getCalls().find((call: { args: unknown[] }) => {
  ...
  it('should NOT replace picklist value in Lead.object when edition is Enterprise', async () => {
    const minimalPackageXml =
      '<?xml version="1.0"?><Package xmlns="http://soap.sforce.com/2006/04/metadata"><types><members>MyApexClass</members><name>ApexClass</name></types><version>58.0</version></Package>';
    const scratchOrgDefFileContent = JSON.stringify({
      Edition: 'Enterprise',
