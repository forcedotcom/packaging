...
import { PackageVersionCreate, PackageVersionCreateUtil } from '../../src/package/packageVersionCreate';
...
describe('Package Version Create', () => {
  ...
  const stubConvert = (): void => {
    ...
    $$.SANDBOX.stub(PackageVersionCreateUtil, 'convertMetadata').resolves({
      packagePath: '/var/folders/lc/yk0hz4l50kq0vs79yb3m_lmm0000gp/T/0Ho3i000000Gmj6XXX-TESTING/md-files',
      converted: [],
    });
    $$.SANDBOX.stub(PackageVersionCreateUtil, 'generateMDFolderForArtifact').resolves();
  };
  ...
  it('should create the package version create request with codecoverage=true', async () => {
    ...
    // @ts-ignore
    const hasUnpackagedMdSpy = $$.SANDBOX.spy(PackageVersionCreateUtil, 'resolveMetadata');
    stubConvert();
    const result = await pvc.createPackageVersion();
  ...
  it('should create the package version create request with codecoverage=false', async () => {
    ...
    const hasSeedMdSpy = $$.SANDBOX.spy(PackageVersionCreateUtil, 'resolveMetadata');
    stubConvert();
